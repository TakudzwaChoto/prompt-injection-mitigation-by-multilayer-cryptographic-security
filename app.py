...
def index():
    ...
    if request.method == "POST":
        ...
        user_msg = {
            ...
            "status": "allowed" if not triggered else "blocked"
        }
        # Strict mode: block if either sanitization or self-checker triggers
        if strict_mode:
            if triggered or not llm_self_check(prompt):
                user_msg["status"] = "blocked"
                session["chat_history"].append(user_msg)
                session.modified = True
                audit_info = {
                    'prompt': prompt,
                    'explanation': 'Strict mode: blocked by sanitization or self-checker.',
                    'status': 'blocked (strict mode)'
                }
                session['audit_info'] = audit_info
                flash("Prompt blocked: Strict mode (sanitization or self-checker).")
                return redirect(url_for("index"))
        else:
            if triggered:
                audit_info = {
                    'prompt': prompt,
                    'explanation': 'Sanitization layer detected suspicious or adversarial content in the prompt.',
                    'status': 'blocked (sanitization)'
                }
                session['audit_info'] = audit_info
                session["chat_history"].append(user_msg)
                session.modified = True
                flash("Prompt blocked: Sanitization layer detected suspicious or adversarial content.")
                return redirect(url_for("index"))
